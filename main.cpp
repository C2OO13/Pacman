...
//Rules and resources for Pacman
...
void init()
{   
	// Resources for lighting & texture in openGL 
...
void DrawInfo() 
{
	...
	std::sprintf(count_buffer + 1,"%d",board->coinsCount); 
...
void display()
{
	...
	// move the pacman
	std::pair<int,int> pacmanCord = pacman->Move();	
	...
	std::pair<int, int> ghostCord0 = ghosts[0]->Move();
	...
	std::pair<int, int> ghostCord1 = ghosts[1]->Move();
	...
	std::pair<int, int> ghostCord2 = ghosts[2]->Move();
	...
	std::pair<int, int> ghostCord3 = ghosts[3]->Move();
	
	vector<pair<int, int>> cords = { ghostCord0, ghostCord1 , ghostCord2, ghostCord3};
	for (int i = 0;i < 4;i++) {
		if (pacmanCord.first == cords[i].first && pacmanCord.second == cords[i].second) {
			cout << "Collision with Ghost at ("<< pacmanCord.first <<","<< pacmanCord.second <<")\n";
			pacman->lives -= 1;
			cout << "Lives left: " << pacman->lives << "\n";
			pacman->tileX = 15;
			pacman->tileY = 2;
			pacman->Reassign(15,2);
			for (int j = 0;j < 4;j++) {
				ghosts[j]->Reassign(13 + j, 10);
				ghosts[j]->tileX = 13 + j;
				ghosts[j]->tileY = 10;
			}
			if (pacman->lives == 0) {
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'c':
		...
		centerDistance = (pacFollowed)? (maxZ - 4) : maxZ;
		if(!pacFollowed)
		{
